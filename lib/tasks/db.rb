...
namespace :db do
  ...
  ENV['DATABASE'] = 'api_sinatra_test' if environment == 'test'
  ENV['DATABASE'] = 'api_sinatra_production' if environment == 'production'
  # connection_string = ENV['DATABASE_URL'] || ENV["DATABASE_URL_#{environment.upcase}"]
  migrations_directory = 'db/migrations'
  connection_string = "postgres://admind:password@localhost/#{ENV['DATABASE']}"
